...
// God of War / Classic Stat Pillar
  ...
  return (
    ...
    >
        ...
        <div className="relative z-10 flex flex-col sm:flex-row sm:items-center justify-between gap-6">
          <div className="flex flex-col gap-1.5">
            <div className="flex items-center gap-2">
              ...
              >
                {theme === 'classic' ? '✦ YOUR CODING ADVENTURE ✦' : '⚔ BASTION OF WAR'}
              </span>
              ...
              >
                LEVEL {stats.level} {theme === 'classic' ? 'ADVENTURER' : 'WARRIOR'}
              </span>
            </div>
            ...
            <h1
              ...
              style={{
                backgroundImage: theme === 'classic'
                  ? 'none'
                  : 'linear-gradient(135deg, #FFFFFF 0%, #F5E6E6 40%, #D4A373 80%, #C59B27 100%)',
                color: theme === 'classic' ? 'var(--theme-text-primary, #0F172A)' : undefined,
                fontFamily: "var(--theme-font-heading, 'Cinzel', serif)",
                filter: theme === 'classic' ? 'none' : 'drop-shadow(0 2px 8px rgba(0,0,0,0.8))',
              }}
            >
              {theme === 'classic' ? `Good day, ${username} ✨` : `Hail, ${username}`}
            </h1>
            ...
            <div className="flex flex-col items-center">
              <span
                ...
                style={{
                  ...
                  fontFamily: "var(--theme-font-heading, 'Cinzel', serif)",
                  filter: theme === 'classic' ? 'none' : 'drop-shadow(0 0 12px rgba(255,61,0,0.5))',
                }}
              >
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        <StatPillar
          icon={Flame}
          label={theme === 'classic' ? "STREAK" : "FURY STREAK"}
          value={stats.streak}
          color={theme === 'classic' ? "var(--theme-accent-glow, #EA580C)" : "#FF3D00"}
          glow="rgba(255,61,0,0.2)"
          subLabel={`${stats.streak} day${stats.streak !== 1 ? 's' : ''} active`}
        />
        <StatPillar
          icon={Star}
          label={theme === 'classic' ? "TOTAL XP" : "HACKSILVER XP"}
          value={stats.xp.toLocaleString()}
          color={theme === 'classic' ? "var(--theme-accent-secondary, #D97706)" : "#F5D060"}
          glow="rgba(245,208,96,0.2)"
          subLabel={`Points Earned`}
        />
        <StatPillar
          icon={Trophy}
          label={theme === 'classic' ? "BADGES" : "TROPHIES"}
          value={badges.filter((b) => b.isUnlocked).length}
          color={theme === 'classic' ? "var(--theme-accent-cyan, #0284C7)" : "#DC2626"}
          glow="rgba(220,38,38,0.2)"
          subLabel={`of ${badges.length} unlocked`}
        />
        <StatPillar
          icon={Zap}
          label={theme === 'classic' ? "LEVEL" : "ASCENSION"}
          value={`Lvl ${stats.level}`}
          color={theme === 'classic' ? "var(--theme-accent-primary, #10B981)" : "#00E5FF"}
          ...
          >
            <div className="flex items-center justify-between mb-2.5">
              <div className="flex items-center gap-2">
                ...
                >
                  {theme === 'classic' ? 'LEVEL PROGRESS' : 'LEVEL ASCENSION'}
                </span>
              </div>
              <span
              ...
              >
                {theme === 'classic' ? `LEVEL ${stats.level}` : `TIER ${stats.level}`}
              </span>
            </div>
