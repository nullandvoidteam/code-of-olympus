...
export const GuidedProjectBuilderWorkspace: React.FC<GuidedProjectBuilderWorkspaceProps> = ({
  ...
  const loadWorkspace = useCallback(
    async (forceRefresh = false) => {
      ...
      try {
        ...
        // Set initial active stage to current stage
        setActiveStage((prevActive) => {
          ...
          const current = res.currentStage || res.stages.find((s: any) => s.is_current) || res.stages.find((s: any) => !s.is_completed && s.is_unlocked) || res.stages[0]

          if (current) {
            const guide = getStageGuide(projectId, current.stage_order, current.title, current.instructions)
            const savedDraft = userId ? localStorage.getItem(`draft_${userId}_${projectId}_${current.id}`) : null
            const initialCode =
              savedDraft ||
            ...
            // Note: Since we are inside a setState callback, we technically shouldn't cause side effects like setCode here,
            // but for simplicity and since it matches the original component design, we'll invoke it directly or rely on the next render.
            setTimeout(() => setCode(initialCode), 0)
          }
  ...
  // Handle selecting a specific stage from the roadmap
  const handleSelectStage = (stage: any) => {
    // Check if locked
    if (!stage.is_unlocked) {
      toast.error('Complete previous stages to unlock this mission!')
      return
    }
    
    // Auto-save current code if leaving a modified active stage
    if (activeStage && code !== activeStage.student_code) {
      handleSaveCode(code, false)
    }

    const guide = getStageGuide(projectId, stage.stage_order, stage.title, stage.instructions)
    const savedDraft = userId ? localStorage.getItem(`draft_${userId}_${projectId}_${stage.id}`) : null
    
    // We start from: 1. Locally saved draft -> 2. Previously submitted code -> 3. Suggested start -> 4. DB starter -> 5. Empty
    const initialCode =
      savedDraft ||
      stage.student_code ||
      guide.suggestedStarterCode ||
  ...
  const stageGuide = activeStage ? getStageGuide(
    projectId,
  ...
  const testCases = activeStage ? getStageTestCases(activeStage.validation_config) : []
  ...
  const nextStage = activeStage ? stages.find((s) => s.stage_order === activeStage.stage_order + 1) : undefined
      ...
      <div className="bg-white rounded-2xl p-4 sm:p-5 border border-stone-200 shadow-xs flex flex-col gap-3">
        ...
        <div className="flex items-center gap-3 pt-1 border-t border-stone-100 text-xs">
          <span className="font-bold text-stone-700 font-pixel text-[11px] uppercase shrink-0">
            {activeStage ? `Stage ${activeStage.stage_order} of ${totalStages}` : `${totalStages} Stages Available`}
          </span>
                ...
                {stages.map((st) => {
                  ...
                  return (
                    <button
                      ...
                      disabled={!isUnlocked}
                      className={`w-full text-left p-2.5 rounded-xl transition-all flex items-start gap-2.5 cursor-pointer border ${isSelected
                    ...
                    >
                      ...
                      <div className="min-w-0 flex-1">
                        <div className="flex items-center justify-between gap-1">
                          <span
                            className={`font-pixel text-[10px] font-bold uppercase ${isSelected
                                ? 'text-purple-700'
                                : isCompleted
                                  ? 'text-emerald-700'
                                  : 'text-stone-500'
              ...
              <div className="px-4 py-2 bg-stone-50 border-b border-stone-200 flex items-center justify-between">
                <div className="flex items-center gap-2">
                  <button
                    ...
                    className={`px-3 py-1 rounded-lg font-pixel uppercase text-[10px] font-bold transition-colors cursor-pointer flex items-center gap-1.5 ${activeBottomTab === 'tests'
                  ...
                  <button
                    ...
                    className={`px-3 py-1 rounded-lg font-pixel uppercase text-[10px] font-bold transition-colors cursor-pointer flex items-center gap-1.5 ${activeBottomTab === 'terminal'
                        ? 'bg-purple-600 text-white shadow-xs'
                        : 'text-stone-600 hover:text-stone-900 bg-stone-200/60'
                      }`}
                  >
                ...
                {activeBottomTab === 'tests' ? (
                  isRunningValidation ? (
                    ...
                    <div className="space-y-3">
                      ...
                      <div
                        className={`p-3 rounded-xl border flex items-center justify-between text-xs font-pixel uppercase font-bold ${submissionResult.passed
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <span>
                            {submissionResult.passed
                              ...
                              : `VALIDATION FAILED • ${submissionResult.testResults.filter((t) => t.passed).length
                      ...
                      <div className="space-y-2">
                        {submissionResult.testResults.map((tr) => (
                          <div
                            key={tr.orderIndex}
                            className={`p-2.5 rounded-xl border flex flex-col gap-1.5 font-mono text-[11px] ${tr.passed
