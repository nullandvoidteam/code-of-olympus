    ...
    <div className="flex flex-col gap-5 text-left">
        ...
        >
          ...
          <div className="flex items-center justify-between gap-4 py-2">
            <div className="flex flex-col min-w-0">
              <span
                className={`text-[11px] font-bold uppercase tracking-wider ${isGow ? 'text-amber-400/80 font-cinzel' : isSpiderman ? 'text-cyan-300/80' : 'text-slate-500'
                  }`}
              >
                {isGow ? 'Your War Clan' : 'Your Squad'}
              </span>
              <span
                className={`font-black text-base sm:text-lg truncate ${isGow ? 'text-[#F5E8E8] font-cinzel' : isSpiderman ? 'text-white' : 'text-slate-900'
                  }`}
              >
                {team.name}
              </span>
      ...
      {pendingIncoming.length > 0 && (
        <div className="flex flex-col gap-3">
          <div
            className={`flex items-center gap-2 text-xs font-black uppercase tracking-wider ${isGow ? 'text-amber-400 font-cinzel' : isSpiderman ? 'text-cyan-400' : 'text-amber-600'
              }`}
          >
            {isGow ? (
            ...
            >
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  <Shield
                    className={`w-4 h-4 ${isGow ? 'text-amber-400' : isSpiderman ? 'text-cyan-400' : 'text-amber-600'
                      }`}
                  />
                  <span
                    className={`font-black text-sm ${isGow ? 'font-cinzel text-rose-100' : isSpiderman ? 'text-white' : 'text-slate-900'
                      }`}
                  >
              ...
              <div
                className={`text-xs font-medium ${isGow ? 'text-stone-300' : isSpiderman ? 'text-blue-200' : 'text-slate-600'
                  }`}
              >
                Rules:{' '}
                <span
                  className={`font-bold capitalize ${isGow ? 'text-amber-300' : isSpiderman ? 'text-cyan-300' : 'text-slate-900'
                    }`}
                >
                  {chal.language}
                </span>{' '}
                •{' '}
                <span
                  className={`font-bold ${isGow ? 'text-rose-200' : isSpiderman ? 'text-white' : 'text-slate-900'
                    }`}
                >
                  {chal.difficulty}
                </span>{' '}
                •{' '}
                <span
                  className={`font-bold ${isGow ? 'text-amber-300' : isSpiderman ? 'text-cyan-300' : 'text-slate-900'
                    }`}
                >
      ...
      {pendingOutgoing.length > 0 && (
        <div className="flex flex-col gap-2.5">
          <span
            className={`text-[11px] font-black uppercase tracking-wider ${isGow ? 'text-amber-400/80 font-cinzel' : isSpiderman ? 'text-cyan-300/80' : 'text-slate-500'
              }`}
          >
          ...
          {pendingOutgoing.map((chal) => (
            ...
            >
              <div className="flex items-center gap-2 truncate">
                ...
                <span
                  className={`font-bold truncate ${isGow ? 'text-rose-100 font-cinzel' : isSpiderman ? 'text-white' : 'text-slate-900'
                    }`}
                >
                  To: {chal.challenged_team?.name || 'Rival Clan'}
                </span>
                <span
                  className={`text-xs font-mono ${isGow ? 'text-amber-300/80' : isSpiderman ? 'text-cyan-300/80' : 'text-slate-500'
                    }`}
                >
      ...
      {recentMatches && recentMatches.length > 0 && (
        <div
          className={`flex flex-col gap-3 pt-4 border-t-2 ${isGow ? 'border-red-950/80' : isSpiderman ? 'border-cyan-900/60' : 'border-slate-100'
            }`}
        >
          <div className="flex items-center justify-between">
            <span
              className={`text-xs font-black uppercase tracking-wider flex items-center gap-1.5 ${isGow ? 'text-amber-400 font-cinzel' : isSpiderman ? 'text-cyan-400' : 'text-slate-700'
                }`}
            >
            ...
            <span
              className={`text-[11px] font-mono ${isGow ? 'text-stone-400' : isSpiderman ? 'text-blue-300/70' : 'text-slate-400'
                }`}
            >
                ...
                >
                  <div className="flex items-center gap-3 min-w-0">
                    <div className="flex flex-col min-w-0">
                      <div className="flex items-center gap-2">
                        <span
                          className={`font-black text-sm truncate ${isGow ? 'font-cinzel text-rose-100' : isSpiderman ? 'text-white' : 'text-slate-900'
                            }`}
                        >
                      ...
                      <span
                        className={`text-[11px] capitalize mt-0.5 font-medium ${isGow ? 'text-stone-400' : isSpiderman ? 'text-blue-200/70' : 'text-slate-500'
                          }`}
                      >
          ...
          >
            ...
            >
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3
                    className={`font-black text-base sm:text-lg tracking-tight ${isGow ? 'text-[#F5E8E8] font-cinzel uppercase' : isSpiderman ? 'text-white uppercase' : 'text-slate-900'
                      }`}
                  >
                    {isGow ? 'Declare War on Rival Clan' : 'Issue Squad Challenge'}
                  </h3>
                  <p
                    className={`text-[11px] font-medium ${isGow ? 'text-stone-400' : isSpiderman ? 'text-blue-200/70' : 'text-slate-500'
                      }`}
                  >
                    {isGow
            ...
            {!selectedTargetTeam ? (
              <div className="flex flex-col gap-3.5 relative z-10">
                <div className="relative">
                  <Search
                    className={`w-4 h-4 absolute left-3.5 top-1/2 -translate-y-1/2 ${isGow ? 'text-red-400' : isSpiderman ? 'text-cyan-400' : 'text-purple-500'
                      }`}
                  />
                  <input
                    type="text"
                ...
                <div className="flex flex-col gap-2.5 max-h-64 overflow-y-auto pr-1">
                  {isSearching ? (
                    <div
                      className={`py-8 flex items-center justify-center gap-2 text-xs ${isGow ? 'text-stone-400' : isSpiderman ? 'text-blue-200' : 'text-slate-500'
                        }`}
                    >
                      <Loader2
                        className={`w-4 h-4 animate-spin ${isGow ? 'text-red-500' : isSpiderman ? 'text-cyan-400' : 'text-purple-600'
                          }`}
                      />
                    ...
                    searchResults.map((t) => (
                      ...
                      >
                        <div className="flex flex-col min-w-0">
                          <div className="flex items-center gap-2">
                            <span
                              className={`font-black text-xs truncate ${isGow ? 'font-cinzel text-rose-100' : isSpiderman ? 'text-white' : 'text-slate-900'
                                }`}
                            >
                              {t.name}
                            </span>
                          ...
                          <span
                            className={`text-[11px] mt-0.5 flex items-center gap-1.5 ${isGow ? 'text-stone-400' : isSpiderman ? 'text-blue-200/70' : 'text-slate-500'
                              }`}
                          >
              ...
              <div className="flex flex-col gap-4 relative z-10">
                ...
                >
                  <div className="flex flex-col">
                    <span
                      className={`text-[10px] uppercase font-black tracking-wider block ${isGow ? 'text-amber-400 font-cinzel' : isSpiderman ? 'text-cyan-300' : 'text-purple-600'
                        }`}
                    >
                    ...
                    <div className="flex items-center gap-2 mt-0.5">
                      <span
                        className={`font-black text-sm sm:text-base ${isGow ? 'font-cinzel text-rose-100' : isSpiderman ? 'text-white' : 'text-slate-900'
                          }`}
                      >
                        {selectedTargetTeam.name}
                      </span>
                      <span
                        className={`text-xs font-mono font-bold ${isGow ? 'text-amber-300' : isSpiderman ? 'text-cyan-300' : 'text-purple-700'
                          }`}
                      >
                ...
                <div className="flex flex-col gap-1.5">
                  <label
                    className={`text-xs font-black uppercase tracking-wider ${isGow ? 'text-amber-400 font-cinzel' : isSpiderman ? 'text-cyan-400' : 'text-slate-700'
                      }`}
                  >
                    Battle Language
                  </label>
                ...
                <div className="flex flex-col gap-2.5">
                  ...
                  <input
                    ...
                    className={`w-full cursor-pointer ${isGow ? 'accent-red-500' : isSpiderman ? 'accent-cyan-400' : 'accent-purple-600'
                      }`}
                  />
                  <div
                    className={`flex justify-between text-[10px] font-mono font-bold ${isGow ? 'text-stone-400 font-cinzel' : isSpiderman ? 'text-blue-300/70' : 'text-slate-400'
                      }`}
                  >
                  ...
                  >
                    ...
                    {isCheckingPool ? (
                      <span className="flex items-center gap-1 text-xs text-slate-400">
                        <Loader2
                          className={`w-3.5 h-3.5 animate-spin ${isGow ? 'text-red-500' : isSpiderman ? 'text-cyan-400' : 'text-purple-600'
                            }`}
                        />{' '}
                        Verifying pool...
                      </span>
