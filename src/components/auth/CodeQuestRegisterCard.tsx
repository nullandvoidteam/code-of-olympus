    ...
    <div className="w-full max-w-[440px] bg-white rounded-3xl p-5 sm:p-6 lg:p-7 border border-slate-100 shadow-[0_20px_50px_rgba(15,23,42,0.07)] flex flex-col transition-all text-left">
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-2.5">
            ...
            <div className="flex items-center gap-2 mt-1 px-0.5">
              ...
              <div className="flex-1 flex gap-1 h-1.5 bg-slate-100 rounded-full">
                {[1, 2, 3].map((step) => (
                  <div
                    key={step}
                    className={`flex-1 rounded-full transition-all duration-300 ${step <= strength.score ? strength.color : 'bg-slate-200'
                      }`}
                  />
                ))}
              </div>
        ...
        <button
          type="submit"
          disabled={isLoading}
          className={`mt-1.5 w-full h-10.5 sm:h-11 text-white font-bold text-sm tracking-wide rounded-xl active:translate-y-0.5 active:shadow-xs transition-all duration-200 flex items-center justify-center gap-2 cursor-pointer disabled:opacity-60 disabled:cursor-not-allowed ${role === 'admin'
